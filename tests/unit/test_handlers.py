...
class TestAddMD5(BaseMD5Test):
    ...
    def test_add_md5_with_file_like_body(self):
        request_dict = {'body': six.BytesIO(b'foobar'), 'headers': {}}
...
@pytest.mark.parametrize(
    'url_original, url_expected',
    [
        # bucket appears twice in path
        (
            'https://s3.us-west-2.amazonaws.com/bkt/bkt/obj.txt?querystr',
            'https://s3.us-west-2.amazonaws.com/bkt/obj.txt?querystr',
        ),
        (
            'https://s3.us-west-2.amazonaws.com/bkt/bkt',
            'https://s3.us-west-2.amazonaws.com/bkt',
        ),
        (
            'https://s3.us-west-2.amazonaws.com/bkt/bkt?query',
            'https://s3.us-west-2.amazonaws.com/bkt?query',
        ),
        (
            'https://bkt.s3.us-west-2.amazonaws.com/bkt/obj.txt?querystr',
            'https://bkt.s3.us-west-2.amazonaws.com/obj.txt?querystr',
        ),
        # bucket appears in path and hostname
        (
            'https://bkt.s3.us-west-2.amazonaws.com/bkt',
            'https://bkt.s3.us-west-2.amazonaws.com/',
        ),
        (
            'https://bkt.s3.us-west-2.amazonaws.com/bkt?querystr',
            'https://bkt.s3.us-west-2.amazonaws.com/?querystr',
        ),
        # keys contains slashes
        (
            'https://bkt.s3.us-west-2.amazonaws.com/bkt/o/b/j.txt',
            'https://bkt.s3.us-west-2.amazonaws.com/o/b/j.txt',
        ),
        # object key contains bucket name
        (
            'https://bkt.s3.us-west-2.amazonaws.com/bkt/bkt/bkt/obj.txt',
            'https://bkt.s3.us-west-2.amazonaws.com/bkt/bkt/obj.txt',
        ),
        # bucket and object contain uppercase and numerical characters
        (
            'https://Bk7.s3.us-west-2.amazonaws.com/Bk7/Bk7/0bJ.txt?querystr',
            'https://Bk7.s3.us-west-2.amazonaws.com/Bk7/0bJ.txt?querystr',
        ),
        # bucket names with dots
        (
            'https://s3.us-west-2.amazonaws.com/b.k.t/b.k.t/obj.txt?querystr',
            'https://s3.us-west-2.amazonaws.com/b.k.t/obj.txt?querystr',
        ),
        (
            'http://b.k.t.s3.us-west-2.amazonaws.com/b.k.t/obj.txt?querystr',
            'http://b.k.t.s3.us-west-2.amazonaws.com/obj.txt?querystr',
        ),
    ],
)
def test_fix_s3_path(url_original, url_expected):
    service_def = {
        'metadata': {
            "endpointPrefix": "s3",
        },
        'operations': {
            'TestOp': {
                'http': {
                    'method': 'POST',
                    'requestUri': '/{Bucket}/{Object}?querystr',
                },
                'name': 'TestOp',
            }
        },
    }
    service_model = ServiceModel(service_def)
    operation_model = service_model.operation_model('TestOp')
    params = {'url': url_original}
    handlers.fix_s3_path(model=operation_model, params=params)
    assert params['url'] == url_expected


def test_fix_s3_path_no_bucket_in_url():
    # When the operation doesn't include the Bucket in the URL, fix_s3_path()
    # should be a noop.
    service_def = {
        'metadata': {
            "endpointPrefix": "s3",
        ...
        'operations': {
            'TestOp': {
                'http': {
                    'method': 'POST',
                    'requestUri': '/{NotBucket}/{Object}?querystr',
                },
                'name': 'TestOp',
            }
        },
    ...
    service_model = ServiceModel(service_def)
    operation_model = service_model.operation_model('TestOp')
    url = 'https://bkt.s3.us-west-2.amazonaws.com/bkt/obj.txt?querystr'
    params = {'url': url}
    handlers.fix_s3_path(model=operation_model, params=params)
    assert params == {'url': url}


def test_fix_s3_path_not_s3():
    # When the service is not S3, fix_s3_path() should be a noop.
    service_def = {
        'metadata': {
            "endpointPrefix": "nots3",
        },
        'operations': {
            'TestOp': {
                'http': {
